...
export async function POST({ request }: { request: Request }) {
	...
	const eventSlug = (data.get("event_slug") ?? "").toString().trim();
	...
	const subject = eventSlug
		? `Event registration: ${eventSlug} — ${name}`
		: `Event registration from ${name}`;
	...
	const htmlBody = `
		<p><strong>Event:</strong> ${eventSlug || "N/A"}</p>
	...
	if (!res.ok) {
		...
		return new Response(JSON.stringify({ error: "Failed to send registration. Please try again later." }), {
			status: 502,
			headers: { "Content-Type": "application/json" },
		});
	}
